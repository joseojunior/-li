...
function Workspace({ user, onLogout, globalError, clearError, onError }: { user: PanelUser; onLogout: () => void; globalError: string; clearError: () => void; onError: (message: string) => void }) {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {section === 'automation' && <><Automation user={user} onError={onError} /><RulesManager user={user} onError={onError} /></>}
...
function Automation({ user, onError }: { user: PanelUser; onError: (message: string) => void }) {
  ...
  return <section className="automation-page"><header className="page-header"><div><p className="eyebrow">Atendimento inteligente</p><h1>Agente e regras</h1><p className="page-description">A operação comercial é controlada por regras, catálogo e transições humanas — não por respostas soltas.</p></div><span className="pill neutral">{loading ? 'Carregando' : 'IA em preparação'}</span></header><div className="automation-overview"><article className="automation-primary"><div><p className="eyebrow">Playbook ativo</p><h2>Guia Estratégico Lilibag</h2><p>Conduz a conversa por contexto, dor, consequência, cenário ideal e recomendação antes de apresentar preço.</p></div><div className="playbook-meta"><strong>v{playbook?.version ?? '—'}</strong><span>{playbook ? 'Versionado e auditável' : 'Aguardando configuração'}</span></div></article><article className="automation-stat"><strong>4</strong><span>etapas de descoberta</span></article><article className="automation-stat"><strong>6</strong><span>situações de handoff</span></article></div><div className="automation-grid"><section className="workflow-card"><header><div><p className="eyebrow">Fluxo de venda</p><h2>Jornada protegida</h2></div></header><ol className="journey-list"><li><span>1</span><div><strong>Contexto</strong><p>Entende o momento e a necessidade com perguntas abertas.</p></div></li><li><span>2</span><div><strong>Dor e cenário ideal</strong><p>Espelha a cliente antes de oferecer qualquer item.</p></div></li><li><span>3</span><div><strong>Produto real</strong><p>Pesquisa catálogo, disponibilidade, preço e mídia.</p></div></li><li><span>4</span><div><strong>Equipe humana</strong><p>Assume pagamento, pedido, dados e situações delicadas.</p></div></li></ol></section><section className="workflow-card safety-card"><header><div><p className="eyebrow">Proteções ativas</p><h2>O agente não pode</h2></div></header><ul className="safety-list"><li>Inventar preço, promoção, frete ou prazo.</li><li>Coletar CPF, endereço, cartão ou pagamento.</li><li>Continuar vendas em situação de luto ou perda.</li><li>Resolver pedidos, devoluções, atacado ou parcerias sozinho.</li></ul><p className="muted">Esses casos são etiquetados e encaminhados automaticamente.</p></section></div>{canManage && playbook && <details className="playbook-details"><summary>Ver instruções ativas do playbook</summary><pre>{playbook.instructions}</pre></details>}</section>;
}
...
function ChannelIntegration({ user, onError }: { user: PanelUser; onError: (message: string) => void }) {
  ...
  return <section className="channel-integration">
    ...
    <div className="channel-body">
      ...
      {selectedChannel ? <>
        <section className="webhook-setup"><header><div><p className="eyebrow">2. Webhook de entrada</p><h3>Gerar URL exclusiva</h3><p>O backend cria um token aleatório por canal, guarda somente o hash e normaliza cada evento antes da fila.</p></div><span className={`pill ${selectedChannel.webhook_configured ? 'success' : 'neutral'}`}>{selectedChannel.webhook_configured ? 'URL criada' : 'Pendente'}</span></header><div className="webhook-setup-fields"><label>Janela de agrupamento<input type="number" min="1" max="30" value={debounceSeconds} onChange={(event) => setDebounceSeconds(event.target.value)} /><small>segundos · padrão recomendado: 5</small></label><div className="webhook-actions"><button className="button secondary" type="button" onClick={() => void saveProcessing()} disabled={savingProcessing}>{savingProcessing ? 'Salvando...' : 'Salvar padrão da fila'}</button><button className="button primary" type="button" onClick={() => void generateWebhook()} disabled={savingWebhook}>{savingWebhook ? 'Gerando...' : selectedChannel.webhook_configured ? 'Gerar nova URL' : 'Gerar URL do webhook'}</button></div></div><p className="queue-explainer"><strong>Fluxo escalável:</strong> API responde rápido, grava o evento com idempotência, agrupa mensagens por {Math.max(1, Math.round(processingDelayMs / 1_000))} s e o worker BullMQ processa em segundo plano com tentativas e bloqueio por conversa.</p></section>
        {webhook && <section className="webhook-result"><header><div><p className="eyebrow">Webhook criado</p><h3>Copie esta URL agora</h3><p>Ela é exclusiva deste canal. O token não pode ser recuperado depois; para trocar a URL, gere uma nova.</p></div><span className="connection-ok">Pronto</span></header>{webhook.webhookUrl ? <div className="webhook-url"><code>{webhook.webhookUrl}</code><button className="button secondary" type="button" onClick={() => void copyWebhook()}>Copiar URL</button></div> : <p className="webhook-warning">Defina `APP_PUBLIC_URL` com a URL pública HTTPS da API antes de usar este webhook fora do ambiente local.</p>}</section>}
