...
export type PromptTestResult = { status: 'waiting_configuration' | 'completed' | 'failed'; agentKey: AgentKey; model: AgentModel; reason?: string; output?: string };
export type BlingConnectionStatus = { status: 'not_configured' | 'pending' | 'active' | 'disabled' | 'error'; clientIdHint?: string; accessTokenExpiresAt?: string | null; createdAt?: string; updatedAt?: string };
...
export const api = {
  ...
  testPrompt: (agentKey: AgentKey, message: string) => request<PromptTestResult>('/v1/panel/ai/prompt-test', { method: 'POST', body: JSON.stringify({ agentKey, message }) })
};
